...
namespace IntermediateExercises
{
  ...
  */
  ...
  {
    private readonly List<object> _stackStore;

    public CustomStack()
    {
      _stackStore = new List<object>();
    }
    ...
    public void Push(object obj)
    {
      ...
      {
        throw new InvalidOperationException(message: "Cannot add null reference to the stack.");
      }
    }

    public object Pop()
    {
      if (_stackStore.Count > 0)
      {
        var obj = _stackStore[_stackStore.Count - 1];
        _stackStore.RemoveAt(_stackStore.Count - 1);
        return obj;
      }
